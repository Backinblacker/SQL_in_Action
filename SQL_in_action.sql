...
-- INVESTIGATE: Should Williamsburg be crowned the most popular neighbourhood? It's a tie between Williamsburg and Harlem

SELECT neighbourhood, count(neighbourhood) AS Popularity
FROM intro_sql.final_airbnb
...
SELECT host_name, count(host_name) as Popular_host 
FROM intro_sql.final_airbnb
group by host_name
order by Popular_host desc;
...
SELECT id, price, (price / 165.10) * 100 - 100 as compare_to_average_price
FROM intro_sql.final_airbnb;